...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
      ...
      var nutFree = _(products).filter(function(x){if(x.containsNuts != true) {return x} });

      var eats = _(nutFree).filter(function(y){
        return _.all(y.ingredients, function(ingredient){ 
          return ingredient != "mushrooms"
        });
      });
      
      productsICanEat.push(eats);
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    ...
    var final = _(sum).chain()
      .filter(function(x){return (x % 3 === 0 || x % 5 ===0)})
      .reduce(function(a, b){return a + b}, 0)
      .value();

    expect(233168).toBe(final);
  });
